...
// const registryUrl = 'https://mirrors.tencent.com/npm/tdesign-react-native';
...
const renderRouter = docRoutes.map((nav, i) => {
  ...
  return (
    <Route
      key={i}
      path={nav.path.replace('/react-native/', '')}
      element={
...
function Components() {
  ...
  //   const historyUrl = `//${version}-tdesign-react-native.surge.sh`;
  ...
  return (
    <td-doc-layout>
      ...
      <td-doc-aside ref={tdDocAsideRef} title="React for React Native">
      ...
      <td-doc-content ref={tdDocContentRef} platform="mobile">
        <Outlet />
        <td-doc-footer slot="doc-footer" platform="mobile" />
      </td-doc-content>
    </td-doc-layout>
  );
}
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="*" element={<Navigate replace to="/react-native/getting-started" />} />
      </Routes>
    </BrowserRouter>
  );
}
